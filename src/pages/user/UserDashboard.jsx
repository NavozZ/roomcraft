      ...
      <div style={{ paddingTop: 64, position: 'relative' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
          ...
          <div ref={cRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: '1.25rem', marginBottom: '2.5rem', ...cStyle }}>
            ...
            {/* View in 3D */}
            {latest ? (
              <Link to={`/user/view3d/${latest.id}`} style={cardBase}
                onMouseEnter={e => { e.currentTarget.style.borderColor='rgba(166,124,82,0.4)'; e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.boxShadow='0 12px 40px rgba(0,0,0,0.3)' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(255,255,255,0.07)'; e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='none' }}>
                {/* Mini room thumbnail */}
                ...
                <span style={{ fontSize: '0.72rem', fontWeight: 600, padding: '4px 12px', borderRadius: 999, background: 'rgba(166,124,82,0.15)', border: '1px solid rgba(166,124,82,0.25)', color: '#C8A882' }}>Open →</span>
              </Link>
            ) : (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: '1rem' }}>
                {designs.map((d, i) => (
                  <Link key={d.id} to={`/user/view3d/${d.id}`}
                    ...
                    <span style={{ fontSize: '0.7rem', color: '#A67C52' }}>→</span>
                  </Link>
                ))}
              </div>
            </div>
