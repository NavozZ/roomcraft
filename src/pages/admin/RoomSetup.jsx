...
function RoomPreview({ room }) {
  ...
  const wallBorder = `3px solid ${wallColour === '#FFFFFF' ? 'rgba(255,255,255,0.5)' : wallColour}`

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <p style={{ fontSize: '0.65rem', color: '#555', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600 }}>Live Preview</p>

      <div style={{ width: maxW + 32, height: maxH + 32, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        {shape === 'l-shape' ? (
          <div style={{ position: 'relative', width: pw, height: ph }}>
            <div style={{ position: 'absolute', top: 0, left: 0, width: pw * 0.6, height: ph, background: floorColour, border: wallBorder, borderRadius: 2 }} />
            <div style={{ position: 'absolute', top: ph * 0.5, left: pw * 0.6, width: pw * 0.4, height: ph * 0.5, background: floorColour, border: wallBorder, borderRadius: 2 }} />
          </div>
        ) : (
          <div style={{ width: pw, height: ph, background: floorColour, border: wallBorder, borderRadius: 2, position: 'relative' }}>
            
            <div style={{ position: 'absolute', inset: 0, opacity: 0.15, backgroundImage: 'linear-gradient(rgba(0,0,0,0.5) 1px,transparent 1px),linear-gradient(90deg,rgba(0,0,0,0.5) 1px,transparent 1px)', backgroundSize: '20px 20px' }} />
            <span style={{ position: 'absolute', bottom: -20, left: '50%', transform: 'translateX(-50%)', fontSize: 9, color: '#555', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>{widthM}m</span>
            <span style={{ position: 'absolute', right: -28, top: '50%', transform: 'translateY(-50%) rotate(90deg)', fontSize: 9, color: '#555', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>{heightM}m</span>
          </div>
        )}
      </div>
...
export default function RoomSetup() {
  ...
  const validateStep1 = () => {
    ...
    if (room.widthM  < 1 || room.widthM  > 30) e.widthM  = 'Width must be 1–30 metres.'
    if (room.heightM < 1 || room.heightM > 30) e.heightM = 'Depth must be 1–30 metres.'
    setErrors(e)
  ...
  const labelStyle = {
    ...
    color: '#777', marginBottom: 8, letterSpacing: '0.06em', textTransform: 'uppercase',
  }
      ...
      <div style={{ paddingTop: 64, position: 'relative' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
          ...
          <div style={{ marginBottom: '2rem', animation: 'rc-up 0.5s ease both' }}>
            <h1 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 'clamp(2rem,5vw,2.8rem)', fontWeight: 600, color: '#FAF7F2', marginBottom: '0.3rem' }}>Set Up Your Room</h1>
            <p style={{ fontSize: '0.875rem', color: '#666' }}>Enter your room details before opening the design editor.</p>
          </div>

          {/* Step indicator */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '2rem', animation: 'rc-up 0.5s ease 0.1s both' }}>
            {['Room Details', 'Colours & Finish'].map((label, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    background: active ? 'rgba(166,124,82,0.2)' : done ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.04)',
                    border: active ? '1px solid rgba(166,124,82,0.4)' : done ? '1px solid rgba(34,197,94,0.3)' : '1px solid rgba(255,255,255,0.08)',
                    color: active ? '#C8A882' : done ? '#4ade80' : '#555',
                    transition: 'all 0.3s',
                  }}>
                    <span style={{ width: 20, height: 20, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 700, background: active ? 'rgba(166,124,82,0.3)' : done ? 'rgba(34,197,94,0.2)' : 'rgba(255,255,255,0.06)' }}>
                      {done ? '✓' : i + 1}
                    </span>
                    {label}
                  </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '1.5rem', alignItems: 'start' }}>
              ...
              {step === 1 && (
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <input type="text" autoFocus placeholder="e.g. Living Room, Master Bedroom"
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '0.75rem', color: '#555', marginBottom: 10 }}>Enter the real-world size of your room.</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      {[['widthM','Width (left → right)'],['heightM','Depth (front → back)']].map(([key, lbl]) => (
                        <div key={key}>
                          ...
                          <div style={{ position: 'relative' }}>
                            <input type="number" min={1} max={30} step={0.5}
                              ...
                              style={{ ...inputStyle(!!errors[key]), paddingRight: 36 }}
                              onFocus={e => { e.target.style.borderColor='rgba(166,124,82,0.5)'; e.target.style.boxShadow='0 0 0 3px rgba(166,124,82,0.1)' }}
                              onBlur={e => { e.target.style.borderColor=errors[key]?'rgba(239,68,68,0.5)':'rgba(255,255,255,0.1)'; e.target.style.boxShadow='none' }}
                            />
                            <span style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', fontSize: '0.75rem', color: '#555' }}>m</span>
                          </div>
                          {errors[key] && <p style={{ fontSize: '0.7rem', color: '#f87171', marginTop: 4 }}>⚠ {errors[key]}</p>}
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      {SHAPES.map(s => (
                        <button key={s.id} type="button" onClick={() => update('shape', s.id)}
                          style={{
                            ...
                            border: room.shape === s.id ? '1px solid rgba(166,124,82,0.6)' : '1px solid rgba(255,255,255,0.08)',
                            background: room.shape === s.id ? 'rgba(166,124,82,0.12)' : 'rgba(255,255,255,0.03)',
                            boxShadow: room.shape === s.id ? '0 0 16px rgba(166,124,82,0.15)' : 'none',
                            transition: 'all 0.2s',
                          }}>
                          ...
                          <span style={{ fontSize: '0.68rem', color: '#555', textAlign: 'center', lineHeight: 1.4 }}>{s.desc}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button onClick={() => setStep(1)}
                      style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: '0.85rem', padding: 0, transition: 'color 0.2s' }}
                      onMouseEnter={e => e.currentTarget.style.color='#FAF7F2'}
                      onMouseLeave={e => e.currentTarget.style.color='#666'}>
                      ← Back
                    </button>
                    <h2 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: '1.6rem', fontWeight: 600, color: '#FAF7F2' }}>Colours & Finish</h2>
                  </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 8 }}>
                      {WALL_COLOURS.map(c => (
                        <button key={c.value} type="button" onClick={() => update('wallColour', c.value)} title={c.label}
                          style={{
                            width: 38, height: 38, borderRadius: 8, background: c.value, cursor: 'pointer',
                            border: room.wallColour === c.value ? '2px solid #A67C52' : '2px solid rgba(255,255,255,0.1)',
                            transform: room.wallColour === c.value ? 'scale(1.15)' : 'scale(1)',
                            boxShadow: room.wallColour === c.value ? '0 0 12px rgba(166,124,82,0.4)' : 'none',
                            transition: 'all 0.2s',
                          }} />
                      ))}
                    </div>
                    <p style={{ fontSize: '0.72rem', color: '#666' }}>
                      Selected: <span style={{ color: '#C8A882' }}>{WALL_COLOURS.find(c => c.value === room.wallColour)?.label}</span>
                    </p>
                  </div>
                      ...
                      {FLOOR_TYPES.map(f => (
                        <button key={f.value} type="button" onClick={() => update('floorColour', f.value)}
                          style={{
                            ...
                            border: room.floorColour === f.value ? '1px solid rgba(166,124,82,0.6)' : '1px solid rgba(255,255,255,0.08)',
                            background: room.floorColour === f.value ? 'rgba(166,124,82,0.1)' : 'rgba(255,255,255,0.03)',
                            transition: 'all 0.2s',
                          }}>
                  ...
                  {/* Summary */}
                  <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '1rem 1.25rem' }}>
                    <p style={{ fontSize: '0.65rem', color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 10 }}>Summary</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 0', fontSize: '0.82rem' }}>
                      {[
                        ['Room name',   room.name],
                        ['Dimensions',  `${room.widthM}m × ${room.heightM}m`],
                        ['Shape',       room.shape],
                        ['Area',        `${(room.widthM * room.heightM).toFixed(1)} m²`],
                      ].map(([k, v]) => (
                        <>
                          <span key={k} style={{ color: '#555' }}>{k}</span>
                          <span key={v} style={{ color: '#C8A882', fontWeight: 500 }}>{v}</span>
                        </>
                      ))}
                    </div>
                  </div>
            ...
            {/* ── Right: preview panel ── */}
            <div style={{ width: 280, flexShrink: 0, animation: 'rc-up 0.5s ease 0.25s both' }}>
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: '1.5rem', position: 'sticky', top: 88 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: '1.25rem' }}>
                  {[`${room.widthM}m × ${room.heightM}m`, room.shape, `${(room.widthM * room.heightM).toFixed(1)} m²`].map(chip => (
                ...
                <p style={{ fontSize: '0.72rem', color: '#444', marginTop: '1rem', lineHeight: 1.6 }}>
                  The canvas will be scaled to match your real room dimensions. Furniture will be placed true to scale.
                </p>
              </div>
            </div>
